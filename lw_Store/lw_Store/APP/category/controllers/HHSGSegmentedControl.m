...
@interface HHSGSegmentedControl ()
...
@implementation HHSGSegmentedControl
...
- (instancetype)initWithFrame:(CGRect)frame delegate:(id<HHSGSegmentedControlDelegate>)delegate segmentedControlType:(SGSegmentedControlType)segmentedControlType titleArr:(NSArray *)titleArr {
...
+ (instancetype)segmentedControlWithFrame:(CGRect)frame delegate:(id<HHSGSegmentedControlDelegate>)delegate segmentedControlType:(SGSegmentedControlType)segmentedControlType titleArr:(NSArray *)titleArr {
    return [[self alloc] initWithFrame:frame delegate:delegate segmentedControlType:segmentedControlType titleArr:titleArr];
}

- (instancetype)initWithFrame:(CGRect)frame delegate:(id<HHSGSegmentedControlDelegate>)delegate segmentedControlType:(SGSegmentedControlType)segmentedControlType nomalImageArr:(NSArray *)nomalImageArr selectedImageArr:(NSArray *)selectedImageArr titleArr:(NSArray *)titleArr {
    if (self = [super initWithFrame:frame]) {
        self.backgroundColor = [[UIColor whiteColor] colorWithAlphaComponent:0.7];
        self.showsHorizontalScrollIndicator = NO;
        self.bounces = NO;
...
+ (instancetype)segmentedControlWithFrame:(CGRect)frame delegate:(id<HHSGSegmentedControlDelegate>)delegate segmentedControlType:(SGSegmentedControlType)segmentedControlType nomalImageArr:(NSArray *)nomalImageArr selectedImageArr:(NSArray *)selectedImageArr titleArr:(NSArray *)titleArr {
...
//        CGFloat button_W = scrollViewWidth / _title_Arr.count;
        ...
        CGSize buttonSize = [self sizeWithText:firstButton.titleLabel.text font:[UIFont systemFontOfSize:btn_fondOfSize] maxSize:CGSizeMake(MAXFLOAT, self.frame.size.height)];
//        _indicatorView.SG_width = buttonSize.width;
        _indicatorView.SG_width = 25;
        _indicatorView.SG_centerX = firstButton.SG_centerX;
        
                self.search_btn = [[UIButton alloc]initWithFrame:CGRectMake(self.mj_w - self.mj_h, 0, self.mj_h, self.mj_h)];
                [self.search_btn setImage:[UIImage imageNamed:@"icon_searchs_default"] forState:UIControlStateNormal];
                [self.search_btn addTarget:self action:@selector(searchAction) forControlEvents:UIControlEventTouchUpInside];
                [self addSubview:self.search_btn];
        
                self.downLine = [UIView lh_viewWithFrame:CGRectMake(self.mj_w - self.mj_h-2, 0, 1, self.mj_h) backColor:KVCBackGroundColor];
...
- (void)searchAction{
    ...
    if ([self.delegate_SG respondsToSelector:@selector(SGSegmentedControlSearchBtnSelected)]) {
        
        [self.delegate_SG SGSegmentedControlSearchBtnSelected];
    }
...
- (void)setSearchBtnHidden:(BOOL)SearchBtnHidden{
    ...
    self.search_btn.hidden = SearchBtnHidden;
    self.downLine.hidden = SearchBtnHidden;
